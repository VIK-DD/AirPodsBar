...
#  Run from the AirPodsBar/ folder: bash install.sh
# ============================================================
...
# --- 0. Check we're in the right folder ---
if [ ! -f "AirPodsBar/main.swift" ]; then
    echo -e "${RED}✗ Error: run this script FROM the AirPodsBar/ folder${NC}"
...
echo -e "${GREEN}✓ Connect/Disconnect uses native IOBluetooth — no external tools needed.${NC}"
...
if command -v iconutil &>/dev/null; then
    ...
    iconutil -c icns "$ICONSET_DIR" -o "$RESOURCES_DIR/AppIcon.icns" 2>/dev/null && \
        echo "  AppIcon.icns created." || echo "  iconutil skipped (PNG used instead)."
fi

echo -e "${GREEN}✓ App bundle ready.${NC}"

# --- 5. Install to /Applications ---
echo -e "\n${YELLOW}[5/6]${NC} Installing to /Applications..."

DEST="/Applications/AirPodsBar.app"
if [ -d "$DEST" ]; then
    echo "  Previous version found — removing..."
...
echo -e "${GREEN}✓ Installed at /Applications/AirPodsBar.app${NC}"
...
# Write plist only — launchd picks it up at next login
# Do NOT call launchctl load — it would spawn a second instance
cp "$(pwd)/com.vik.airpodsbar.plist" "$PLIST_DEST"

echo -e "${GREEN}✓ Launch at login configured (takes effect on next login).${NC}"
...
echo -e "${GREEN}================================${NC}"
echo ""
echo "  Launching AirPodsBar..."
open "$DEST"
sleep 1
echo ""
echo -e "  🎧 The icon is now in your menu bar (top right)."
echo -e "  Click it to view your AirPods battery levels."
echo ""
echo -e "${BLUE}Useful commands:${NC}"
echo "  Stop app:             pkill -x AirPodsBar"
echo "  Disable auto-launch:  launchctl unload ~/Library/LaunchAgents/com.vik.airpodsbar.plist"
